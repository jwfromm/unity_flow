...
class UnityModel(object):
    ...
    model_path : Optional[Union[str, Path]]
        The path to a saved UnityModel, one of exe and model_path must
        be specified.
    ...
    def save(self, model_path: Union[str, Path]):
        ...
        model_path : Union[str, Path]
            A full path to save this UnityModel to including the output file name.
    ...
    def load(self, model_path: Union[str, Path]) -> Tuple[relax.Executable, Dict[List, str]]:
        """Load a saved UnityModel back into memory.

        Parameters
        ----------
        model_path : Union[str, Path]
            The path to the saved UnityModel that will be loaded.
